...
SELECT c.fname AS Имя, c.lname AS Фамилия, c.date_of_birth AS Дата_рождения, m.name AS Фильм, mm.member_type AS Профессия FROM castmembers AS c 
INNER JOIN movies_members AS mm ON c.cm_id = mm.cm_id JOIN movies AS m ON mm.movie_id = m.movie_id;
...
SELECT m.movie_id, m.name, m.year, m.country, g.genre, c.cm_id, c.fname, c.lname, c.mname, c.date_of_birth, u.fname, u.lname, r.review_id, r.rank, r.comment FROM movies AS m INNER JOIN movies_genre AS mg 
ON m.movie_id = mg.movie_id JOIN genre AS g ON mg.genre_id = g.genre_id JOIN movies_members AS mm ON m.movie_id = mm.movie_id 
JOIN castmembers AS c ON mm.cm_id = c.cm_id LEFT JOIN reviews AS r ON r.movie_id = m.movie_id LEFT JOIN users AS u ON u.user_id = r.user_id ORDER BY m.movie_id;